...
| `iris_min_darkness` | float | `0.05` | 0.0 – 1.0 | (iris) The iris/pupil must be darker than the surrounding eyeball. Minimum interior-vs-ring darkness margin (normalized; `1.0` ≈ 128 gray levels), measured with specular highlights excluded so a glint can't fool it. Raise to reject bright reflections; the chosen candidate's value shows as `dark=` in the overlay. The darkest round candidate is also preferred during scoring. |
...
| `eye_center_offset_x` | float | `0` | -200 – 200 | (iris/boxiris) Manual correction for the eye-box anchor, in **source** pixels. Positive moves the center right in the (mirror-aware) displayed image. Use to cancel the Haar box's slight nasal bias; tune per eye. Shifts the eye box/follow anchor only — the iris is unaffected. |
| `eye_center_offset_y` | float | `0` | -200 – 200 | (iris/boxiris) Vertical companion to `eye_center_offset_x`; positive moves the center down. |
